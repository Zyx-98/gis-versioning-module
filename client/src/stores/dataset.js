...
export const useDatasetStore = defineStore("dataset", {
  ...
  getters: {
    ...
    activeBranches: (state) =>
      state.branches.filter((b) => b.status === "active" && !b.isMain),
    pendingMergeRequests: (state) =>
      state.mergeRequests.filter((mr) => mr.status === "pending"),
  },
  ...
  actions: {
    ...
    async createDataset(data) {
      ...
      try {
        ...
        this.error =
          error.response?.data?.message || "Failed to create dataset";
        throw error;
      } finally {
        this.loading = false;
      }
    ...
    async checkoutBranch(datasetId, branchName) {
      ...
      try {
        ...
        this.error =
          error.response?.data?.message || "Failed to checkout branch";
        throw error;
      } finally {
        this.loading = false;
      }
    ...
    async fetchBranchChanges(branchId) {
      try {
        const response = await api.getBranchChanges(branchId);
        this.branchChanges = response.data;
        return response.data;
      } catch (error) {
        console.error("Failed to fetch branch changes:", error);
        throw error;
      }
    },

    async checkForMainUpdates(branchId) {
      try {
        const response = await api.checkForUpdates(branchId);
        this.mainUpdates = response.data;
        return response.data;
      } catch (error) {
